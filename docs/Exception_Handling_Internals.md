# Exception Handling Internals & Architecture
...
### Frame Unwinding Logic
```cpp
while (frames.size() > tb.frameIdx + 1) {
    closeUpvalues(frames.back().slots); // Prevent dangling stack references
    frames.pop_back();
    frameUpvalues.pop_back();
}
stackTop = tb.stackTop; // Clean up intermediate stack temporaries
LOAD_FRAME();
ip = tb.catchIp;        // Re-route instruction pointer to catch handler
*stackTop++ = exc;      // Push exception for catch parameter binding
DISPATCH();             // Continue computed goto dispatch
```
...
1. **No C++ Exception Leaks Across C ABIs**:
   Throwing C++ exceptions (`throw std::runtime_error`) across `libuv` C event loop callbacks or native Win32 FFI function pointers (`WndProc`, `WNDCLASS`) causes instant process termination or corrupted stack unwinding tables.
2. **Safe Fault Routing (`handle_vm_fault`)**:
   When a native helper or VM instruction faults, it sets `pendingException` and routes via `goto handle_vm_fault` within the local `run()` loop, keeping the C call stack intact and unwinding only EZ virtual frames.

---

## 7. 🚀 Future Optimization: Zero-Cost Static Exception Tables

- **Current Cost**: Every `try` block dynamically executes `TRY_START` (allocating on `tryStack`) and `TRY_END` (popping from `tryStack`), adding overhead inside tight loops.
- **Zero-Cost Design**:
  Move `TryBlock` definitions into a compile-time static `ExceptionTable` in each `Chunk`:
  ```cpp
  ...
  ```
  - **Happy Path**: When no exceptions occur, `try` blocks execute with **zero CPU instructions and zero vector allocations**.
  - **Fault Path**: When an exception occurs, the VM performs a binary search on the `ExceptionTable` by current `ip`.
