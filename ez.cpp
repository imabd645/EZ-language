...
// 1. Simple templating: replace {{key}} with value
    ...
    // ==================== TERMINAL CONTROL ====================
        ...
        builtins["http_get"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() < 1 || args.size() > 2)
                error("http_get expects 1 or 2 arguments (url, [headers_struct])", line);
            
            string url = args[0].toString();
            CURL* curl = curl_easy_init();
            if (!curl) error("Failed to initialize CURL", line);
            
            string response;
            struct curl_slist* headers = nullptr;
            
            // Add custom headers if provided
            if (args.size() == 2 && args[1].type == Value::STRUCT)
            {
                for (const auto& pair : args[1].structFields)
                {
                    string header = pair.first + ": " + pair.second.toString();
                    headers = curl_slist_append(headers, header.c_str());
                }
            }
            
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, +[](void* contents, size_t size, size_t nmemb, string* userp) -> size_t {
                size_t totalSize = size * nmemb;
                userp->append((char*)contents, totalSize);
                return totalSize;
            });
            ...
            if (res != CURLE_OK)
                error("HTTP GET failed: " + string(curl_easy_strerror(res)), line);
            
            return Value::String(response);
        };
        
        // http_post() - Make HTTP POST request
        builtins["http_post"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() < 2 || args.size() > 3)
                error("http_post expects 2 or 3 arguments (url, body, [headers_struct])", line);
            ...
            // Add custom headers if provided
            if (args.size() == 3 && args[2].type == Value::STRUCT)
            {
                for (const auto& pair : args[2].structFields)
                {
                    string header = pair.first + ": " + pair.second.toString();
                    headers = curl_slist_append(headers, header.c_str());
                }
            ...
            if (res != CURLE_OK)
                error("HTTP POST failed: " + string(curl_easy_strerror(res)), line);
            
            return Value::String(response);
        };
        
        // http_put() - Make HTTP PUT request
        builtins["http_put"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() < 2 || args.size() > 3)
                error("http_put expects 2 or 3 arguments (url, body, [headers_struct])", line);
            ...
            if (res != CURLE_OK)
                error("HTTP PUT failed: " + string(curl_easy_strerror(res)), line);
            
            return Value::String(response);
        };
        
        // http_delete() - Make HTTP DELETE request
        builtins["http_delete"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() < 1 || args.size() > 2)
                error("http_delete expects 1 or 2 arguments (url, [headers_struct])", line);
            ...
            if (res != CURLE_OK)
                error("HTTP DELETE failed: " + string(curl_easy_strerror(res)), line);
            
            return Value::String(response);
        };
        
        // parse_json() - Parse JSON string to EZ struct/array
        builtins["parse_json"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() != 1)
                error("parse_json expects 1 argument (json_string)", line);
            ...
            if (!reader.parse(jsonStr, root))
                error("Failed to parse JSON: " + reader.getFormattedErrorMessages(), line);
            
            // Convert JSON to EZ Value recursively
            function<Value(const Json::Value&)> jsonToValue = [&](const Json::Value& json) -> Value {
                if (json.isNull()) return Value::String("");
                if (json.isBool()) return Value::Bool(json.asBool());
                if (json.isInt()) return Value::Number(json.asInt());
                if (json.isDouble()) return Value::Number(json.asDouble());
                if (json.isString()) return Value::String(json.asString());
                ...
                if (json.isObject()) {
                    unordered_map<string, Value> obj;
                    for (const auto& key : json.getMemberNames()) {
                        obj[key] = jsonToValue(json[key]);
                    }
                    return Value::Struct(obj);
                }
                
                return Value::String("");
            };
            
            return jsonToValue(root);
        };
        
        // to_json() - Convert EZ struct/array to JSON string
        builtins["to_json"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() != 1)
                error("to_json expects 1 argument (value)", line);
            
            // Convert EZ Value to JSON recursively
            function<Json::Value(const Value&)> valueToJson = [&](const Value& val) -> Json::Value {
                if (val.type == Value::NUM) {
                    return Json::Value(val.num);
                }
                if (val.type == Value::STR) {
                    return Json::Value(val.str);
                }
                if (val.type == Value::T_BOOL) {
                    return Json::Value(val.boolean);
                }
                if (val.type == Value::ARR) {
                    Json::Value arr(Json::arrayValue);
                    for (const auto& item : val.arr) {
                        arr.append(valueToJson(item));
                    }
                    return arr;
                }
                if (val.type == Value::STRUCT) {
                    Json::Value obj(Json::objectValue);
                    for (const auto& pair : val.structFields) {
                        obj[pair.first] = valueToJson(pair.second);
                    }
                    return obj;
                }
            ...
            Json::Value jsonVal = valueToJson(args[0]);
            Json::StreamWriterBuilder builder;
            builder["indentation"] = "";
            return Value::String(Json::writeString(builder, jsonVal));
        };
        
        // url_encode() - URL encode a string
        builtins["url_encode"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() != 1)
                error("url_encode expects 1 argument (string)", line);
        ...
        builtins["url_decode"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() != 1)
                error("url_decode expects 1 argument (string)", line);
            ...
            int outLength;
            char* decoded = curl_easy_unescape(curl, str.c_str(), str.length(), &outLength);
            string result(decoded, outLength);
            curl_free(decoded);
            curl_easy_cleanup(curl);
        ...
        // parse_query_string() - Parse URL query string to struct
        builtins["parse_query_string"] = [](vector<Value> &args, int line) -> Value
        {
            if (args.size() != 1)
                error("parse_query_string expects 1 argument (query_string)", line);
            ...
            while (getline(ss, pair, '&')) {
                ...
                if (eq != string::npos) {
                    ...
                    if (curl) {
                        int len;
                        char* decoded = curl_easy_unescape(curl, value.c_str(), value.length(), &len);
                        result[key] = Value::String(string(decoded, len));
                        curl_free(decoded);
                        curl_easy_cleanup(curl);
                    }
                }
